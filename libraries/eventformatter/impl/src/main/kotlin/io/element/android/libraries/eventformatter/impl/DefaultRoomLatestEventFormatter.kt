...
import io.element.android.libraries.matrix.ui.messages.simplified
...
class DefaultRoomLatestEventFormatter(
    ...
    private fun formatContent(
        ...
        return when (content) {
            ...
            is StateContent -> {
                stateContentFormatter.format(content, senderDisambiguatedDisplayName, senderId, isOutgoing, RenderingMode.RoomList)
            }
            is PollContent -> {
    ...
    private fun MessageContent.process(
        ...
        val message = when (val messageType: MessageType = type) {
            ...
            is TextMessageType -> {
                messageType.simplified().toPlainText(permalinkParser)
            }
            is VideoMessageType -> {
