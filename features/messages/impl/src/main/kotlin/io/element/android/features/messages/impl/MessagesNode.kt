...
class MessagesNode(
    ...
    private fun onLinkClick(
        ...
        when (val permalink = permalinkParser.parse(url)) {
            is PermalinkData.UserLink -> {
                ...
                callback.navigateToRoomMemberDetails(permalink.userId, null)
            }
        ...
        CompositionLocalProvider(
            ...
            roomMemberModerationRenderer.Render(
                ...
                onSelectAction = { action, target ->
                    when (action) {
                        is ModerationAction.DisplayProfile -> callback.navigateToRoomMemberDetails(target.userId, target.primaryZIdOrWalletAddress)
    ...
    private fun CoroutineScope.getUserInfo(userId: UserId) = launch {
        room.getUpdatedMember(userId)
            .onSuccess { user ->
                callback.navigateToRoomMemberDetails(userId, user.primaryZId)
            }
            .onFailure {
                callback.navigateToRoomMemberDetails(userId, null)
            }
    }
}
